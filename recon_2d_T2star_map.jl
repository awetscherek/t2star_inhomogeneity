...
function recon_2d_t2star_map(config, kx, ky, raw, time_since_last_rf, dims; # keyword arguments: 
    ...
    g_r2 = combine_coils ? Array{Float64}(undef, size(r2_d)) : Array{Float64}(undef, nx, ny, nz * config["nchan"]);
    g_b0 = combine_coils ? Array{Float64}(undef, size(Δb0_d)) : Array{Float64}(undef, nx, ny, nz * config["nchan"]);
