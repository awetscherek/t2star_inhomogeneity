...
function recon_2d_t2star_map(config, kx, ky, raw, time_since_last_rf, dims; # keyword arguments: 
    ...
    #Erroring here - need to change selection
    ...
    #find shape of time_since_last_rf
    println(size(time_since_last_rf))
    time_since_last_rf = vec(permutedims(time_since_last_rf, [4 3 1 2]))
    ...
    for t = 1:timepoints
        ...
        finufft_setpts!(plan2, kx_d_t[selection], ky_d_t[selection])
        plan1 = finufft_makeplan(1, dims, -1, nz * config["nchan"], tol)    # type 1 (adjoint transform)
        finufft_setpts!(plan1, kx_d_t[selection], ky_d_t[selection])
        ...
        w_d_t = s0_d * Base.e ^ ((t_ms ./ t2_d))

        finufft_exec!(plan2, w_d_t .* c_d, r)
        r_t .*= dcf_d;
        r_t .-= y_d[t];
