...
function calculate_synthetic_coil_sensitivity(config, eval_no, kx, ky, y_d)
    ...
    if !isfile("data/coil_sens/$(eval_no)_synthetic_sens.cfl")
        ...
        run(`../../bart-0.9.00/bart fft -u 7 data/coil_sens/lowres_img data/coil_sens/lowres_ksp`)
        run(`../../bart-0.9.00/bart resize -c 0 $nx 1 $ny 2 $nz data/coil_sens/lowres_ksp data/coil_sens/ksp_zerop`)
        run(`../../bart-0.9.00/bart ecalib -t 0.01 -m1 data/coil_sens/ksp_zerop data/coil_sens/$(eval_no)_synthetic_sens`)
    end
    # load coil sensitivities into Julia
    return ReadWriteCFL.readcfl("data/coil_sens/$(eval_no)_synthetic_sens.cfl")
end