function forward_operator_impl(plan2, r2_d, b0_d, s0_fat_d, s0_water_d, num_timepoints, num_total_timepoints, kx_d, ky_d,
    c_d, timepoints, selection, timepoint_window_size, fat_modulation = nothing)
    if !isnothing(fat_modulation)
        y_water = _forward_operator_impl(plan2, r2_d, b0_d, s0_water_d, num_timepoints, num_total_timepoints, kx_d, ky_d,
        c_d, timepoints, selection, timepoint_window_size)
        y_fat = _forward_operator_impl(plan2, r2_d, b0_d, s0_fat_d, num_timepoints, num_total_timepoints, kx_d, ky_d,
        c_d, timepoints, selection, timepoint_window_size)
        return y_water .+ fat_modulation .* y_fat
    else
        # Not utilising Fat Modulation, implicitly assume that everything is Water
        return _forward_operator_impl(plan2, r2_d, b0_d, s0_water_d, num_timepoints, num_total_timepoints, kx_d, ky_d,
        c_d, timepoints, selection, timepoint_window_size)
    end
end

function _forward_operator_impl(plan2, r2_d, b0_d, s0_d, num_timepoints, num_total_timepoints, kx_d, ky_d,
    c_d, timepoints, selection, timepoint_window_size)
