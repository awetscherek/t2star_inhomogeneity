function adjoint_operator_impl(plan1, r, e_d, s0_fat_d, s0_water_d, dcf_d, combine_coils, c_d, num_timepoints, num_total_timepoints,
    timepoints, kx_d, ky_d, selection, use_dcf, timepoint_window_size, fat_modulation, nchan)
    ...
    for t in ProgressBar(1:num_timepoints)
        ...
        if combine_coils
            ...
            g_r_fat_result_t = reshape(g_r_fat_t, size(e_d))
            g_r_water_result_t = reshape(g_r_water_t, size(e_d))
        end
        ...
        g_e_total .+= ((-conj_s0_water .* t_ms .* conj_exp_term) .* g_r_water_result_t) 
    ...
    return g_e_total, g_s0_fat_total, g_s0_water_total
end
